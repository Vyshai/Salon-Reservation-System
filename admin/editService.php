...
elseif ($_SERVER["REQUEST_METHOD"] == "POST") {
    $service_id = trim(htmlspecialchars($_POST["id"]));
    ...
    if (empty(array_filter($error))) {
        if ($serviceObj->serviceExists($service["service_name"], $service_id)) {
            ...
            if ($serviceObj->editService($service_id)) {
                $submit_success = "Service updated successfully!";
            } else {
                $submit_error = "Failed to update service. Please try again.";
            }
        }
    } else {
...
<body>
    <div class="container">
        ...
        <form action="" method="post">
            ...
            <input type="text" name="service_name" id="service_name" value="<?= isset($service["service_name"]) ? $service["service_name"] : ''; ?>">
            <p class="error"><?= $error["service_name"]; ?></p>

            <label for="description">Description <span>*</span></label>
            <textarea name="description" id="description"><?= isset($service["description"]) ? $service["description"] : ''; ?></textarea>
            <p class="error"><?= $error["description"]; ?></p>

            <label for="price">Price (₱) <span>*</span></label>
            <input type="number" name="price" id="price" step="0.01" value="<?= isset($service["price"]) ? $service["price"] : ''; ?>">
            <p class="error"><?= $error["price"]; ?></p>

            <label for="duration">Duration (minutes) <span>*</span></label>
            <input type="number" name="duration" id="duration" value="<?= isset($service["duration"]) ? $service["duration"] : ''; ?>">
